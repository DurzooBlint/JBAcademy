...
class Path:
    ...
    def store_move(self, i_row, i_column, i_score):
        self.moves.append([i_row, i_column])
        self.moves.append(i_score)
...
class Board:
    ...
    def insert_cell_minmax(self, s_cords):
        if s_cords[-1] != 'c':
            l_cords = s_cords.split(" ")
            i_row_t = int(l_cords[0])
            i_column_t = int(l_cords[1])
...
def computer_moves(board, s_level):
    ...
    if s_level == "easy":
        ...
        i_row, i_column = minimax(board, board.what_is_next())
...
def validate_command(s_input):
    b_result = False
    l_choices = ["user", "easy", "medium", "hard"]
...
def translate_score(s_result, s_who):
    if s_who == "X":
        s_opponent = 'O'
    else:
        s_opponent = "X"

    if s_result == s_who + " wins":
        i_points = 10
    elif s_result == s_opponent + " wins":
        i_points = -10
    elif s_result == "Draw":
        i_points = 0
    else:
...
def minimax(board, s_who):
    ...
    i = 0
    while i < len(new_board.l_board):
        j = 0
        while j < len(new_board.l_board[i]):
            if new_board.l_board[i][j] == " ":
                new_board.insert_cell_minmax(str(i) + " " + str(j))
                score = translate_score(new_board.get_score(), s_who)
                path.store_move(i, j, score)
                minimax(new_board, s_who)
            j += 1
        i += 1
    return path
