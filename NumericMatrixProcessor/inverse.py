...
def minor_2(l_matrix, i_row, i_column):
    l_result = []
    i = 0
    while i < len(l_matrix):
        if i != i_row:
            j = 0
            l_row = []
            while j < len(l_matrix[i]):
                if j != i_column:
                    l_row.append(l_matrix[i][j])
                j += 1
            ...
            if len(l_matrix) == 2:
                ...
                while i < len(l_matrix[0]):
                    if (i + 1) % 2 == 1:
                        ...
                        i_sign = -1
                    i_result += i_sign * l_matrix[0][i] * determinant(minor_2(l_matrix, 0, i))
                    i += 1
        else:
            print('Error')
            i_result = -1
...
def matrix_of_cofactors(l_matrix):
    l_result = []
    i = 0
    while i < len(l_matrix):
        j = 0
        l_row = []
        while j < len(l_matrix[i]):
            l_row.append(determinant(minor_2(l_matrix, i, j)) * get_sign(l_matrix, i, j))
            j += 1
        l_result.append(l_row)
        i += 1
...
def generate_signs_matrix(l_matrix):
    l_result = []
    i = 0
    while i < len(l_matrix):
        j = 0
...
def get_sign(l_matrix, i_row, i_column):
    l_signs = generate_signs_matrix(l_matrix)
    i_sign = l_signs[i_row][i_column]
    return i_sign


def inverse(l_matrix):
    i_det_matrix = determinant(l_matrix)

    if i_det_matrix == 0:
        print("This matrix doesn't have an inverse.")
    else:
        l_cofactors_matrix = matrix_of_cofactors(l_matrix)
        l_cofactors_matrix_transposed = transpose(l_cofactors_matrix, 1)
        l_result = multiply_matrix_constant(l_cofactors_matrix_transposed, 1 / i_det_matrix)
        return l_result
